...
import { doPost, doPut } from "../request";
import APIPaths from "../api-paths";
import { RequestAccess } from "../request_jwt_e";
import { RequestError } from "../request-error";

export const loadProducts = (): TE.TaskEither<RequestError, Product[]> =>
  pipe(
    doPost<ProductsResponse>(APIPaths.Product.List),
    TE.map((response: ProductsResponse) => response.products),
  );

export const loadProductById = (id: string): TE.TaskEither<RequestError, Product> =>
  pipe(
...
export const saveTaxamoResponse = (paymentData: TaxamoPaymentResult): TE.TaskEither<RequestError, EmptyResponse> => doPost<EmptyResponse>(APIPaths.Payment.SaveTaxamoResponse, { t: paymentData });
